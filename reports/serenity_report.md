...
```text
Serenity Score =
需求确定性 + 成本冲击 + 失败成本跃迁 + 强制采用 + 瓶颈强度 + 证据强度 + 公司纯度 + 扩产难度 + 客户验证 + 旧标签错配 + 市场未定价 + 时机
- 涨幅惩罚 - 普通拥挤度 - A股拥挤度 - 稀释风险 - 财务风险 - 技术替代风险 - 证据断裂风险

硬门槛：
融资依赖 HIGH 封顶 35；交易对手质量 LOW 封顶 50；交易可达性 LIMITED 封顶 60；INACCESSIBLE 归零。

涨幅惩罚：
300% 以上触发中等惩罚，500% 以上才进入重罚，800% 以上只跟踪不挖掘。
```
